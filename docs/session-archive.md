...
| `bench.baseline_total_us` / `candidate_total_us` | Aggregated at archive time from per-invocation `verify/<target>/<inv>/bench-run.json` (target calibration baseline) + `optimize/<target>/<inv>/bench-run.json` (verification bench) via `.data.summary.total_duration_us`. Targets with a `verification_replay` get real totals; targets without it (full-range fallback path) keep totals at 0 — landed v5 Phase 3. | — |
