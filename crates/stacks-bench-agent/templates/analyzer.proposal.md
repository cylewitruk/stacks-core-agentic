...
does not move. Prefer smaller numbers when uncertain.
...
forks. Prefer single-block or small-contiguous-range evidence for throughput
findings. If high-cost behavior is concentrated in early blocks and absent from
late blocks, it may be an old epoch bug already fixed by recalibration; reject
with that reason rather than proposing work.
