...
pub async fn run(args: OptimizeRunArgs, ctx: &CliContext, session_id: &SessionId) -> Result<()> {
    ...
    // The standalone `session optimize run` consumes the per-session
    // source checkout previously materialized by `session run`. Read
    // source.json — it's the truth — and derive the per-session
    // checkout path from the RECORDED cache_id, NOT from current
    // settings (an operator who changes/removes `[source].id` between
    // invocations must still find the original checkout). Bails with
    // a clear pointer if no prior `session run` has materialized the
    // source for this session.
    let source =
        crate::models::source::SourceJson::read(&layout.source_json()).with_context(|| {
            "per-session source.json missing — run `sbagent session run` first to materialize the \
             source checkout (or remove the partial session dir to start over)"
        })?;
