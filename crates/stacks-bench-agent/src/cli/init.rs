...
//!    `stacks_core.base_repo_url`, `publish.base_branch`,
//!    `layout.prompt_overrides_dir`). If `--push` OR `--seed-from` is set, also
//!    `publish.token_file` (both rely on the bot's PAT to authenticate against
//!    GitHub).
...
//!    `publish.base_branch` from the source URL and push it to
//!    `stacks_core.base_repo_url` via PAT-via-env. Required when the bot fork is
//!    brand-new and doesn't yet carry the substrate branch.
//!    `stacks_core.base_repo_url` must start with the configured
//!    [`Settings::effective_git_auth_url_prefix`] (defaults to
//!    `https://github.com/`); the auth header only matches HTTPS URLs
//!    under that prefix.
//! 4. `git init` if no `.git/` is present.
//! 5. Add `base` as a submodule from `stacks_core.base_repo_url`, checked out
//!    at `publish.base_branch`. Skipped if the submodule entry already exists.
//! 6. Replicate the bot remote on the submodule (best-effort, derived from
//!    `publish.head_owner` / `publish.base_repo`).
...
//!    preserve operator tunes. Default layout: `<layout.prompt_overrides_dir>`
//!    (typically `.sbagent/prompts/`) + the sibling `.sbagent/schemas/` derived
//!    from its parent when `layout.schemas_dir` is unset in config.
...
//!    `<layout.prompt_overrides_dir>`, `<layout.schemas_dir>`) — never `git add
//!    -A` — then commit "chore: initial operator state" authored as the bot via
...
//!     configured `git.auth_url_prefix` (default `https://github.com/`).
...
pub struct InitArgs {
    ...
    /// Seed `publish.base_branch` on the bot fork (`stacks_core.base_repo_url`)
    /// from a different upstream fork BEFORE `git submodule add` runs.
    /// Required on the very first init against a brand-new bot fork
    /// that doesn't yet carry the configured `publish.base_branch` —
    ...
    /// `stacks_core.base_repo_url` using the same PAT-via-env mechanism as
    /// `--push`, so the same `publish.token_file` precondition applies.
    ///
...
pub async fn run(args: InitArgs, settings: &Settings) -> Result<()> {
    ...
    let base_path = settings
        ...
        .as_deref()
        .context(
            "`stacks_core.base` not set in config; required for init (relative path under the \
             operator dir for the stacks-core submodule)",
        )?;
    let base_repo_url = settings
        ...
        .as_deref()
        .context(
            "`stacks_core.base_repo_url` not set in config; required for init (clone URL for the \
             stacks-core submodule)",
        )?;
    let base_branch = settings
        ...
        .as_deref()
        .context(
            "`publish.base_branch` not set in config; required for init (the branch the submodule \
    ...
    let token: Option<String> = if needs_token {
        let token_file = settings
            .publish
            .token_file_required()
            .context("`sbagent init --push` / `--seed-from`")?;
    ...
    // Step 0 (optional): seed `<publish.base_branch>` on the bot fork
    // (`stacks_core.base_repo_url`) from `--seed-from`. Required when the bot fork
    ...
    if let Some(seed_url) = args.seed_from.as_deref() {
        // The seed step PUSHes to `stacks_core.base_repo_url` using the
        // PAT-via-extraheader mechanism, scoped to
        // `http.<auth_url_prefix>.extraheader`. If `stacks_core.base_repo_url`
        ...
        validate_auth_url(
            base_repo_url,
            &auth_url_prefix,
            "`--seed-from`'s `stacks_core.base_repo_url`",
        )?;
        let tok = token
            .as_deref()
    ...
    // `publish.base_repo`. Best-effort — if the operator is on
    // upstream-mode (`publish.base_repo` already canonical), this
    // still adds a `bot` remote pointing at the bot's fork (which we
    // derive from `publish.head_owner`).
...
/// Derive the bot's stacks-core fork URL from `publish.head_owner` (or
/// fall back to `publish.base_repo` if it looks like the bot's own
...
fn derive_bot_fork_url(settings: &Settings) -> Option<String> {
    // Prefer `publish.head_owner` (explicit "this is the bot"); fall
    // back to `publish.base_repo` for pilot mode where head + base are
    // both the bot's fork.
    settings
        .publish
        .head_owner
...
fn write_default_gitignore(target_dir: &Path, base_path: &Path) -> Result<()> {
    ...
    let body = format!(
        "# Defensive: not auto-loaded (config lives at ~/.config/sbagent/config.toml),\n# but \
         ...
         `layout.agent_workspace_root` in /private/tmp/... by default,\n# but old layouts kept it \
         here.\n/sessions/*/worktrees/\n\n# Editor / OS \
         noise.\n.DS_Store\n.idea/\n.vscode/\n*.swp\n"
    );
...
/// the `git.auth_username` / `git.auth_url_prefix` settings flow
/// through end-to-end.
...
/// `publish.base_branch`. Without this step, `git submodule add -b
